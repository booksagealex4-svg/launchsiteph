...
/** Only Overview has a real route so far — every other item is a clearly inert placeholder until its page is built. */
const navItems: AdminNavItem[] = [
  { label: 'Overview', icon: LayoutDashboard, to: '/admin' },
  { label: 'Clients', icon: Users, to: '/admin/clients' },
  { label: 'Projects', icon: Briefcase, to: '/admin/projects' },
  { label: 'Reviews', icon: Eye, to: '/admin/reviews' },
  { label: 'Messages', icon: MessageCircle, to: '/admin/messages' },
  { label: 'Leads', icon: Target, to: '/admin/leads' },
  { label: 'Payments', icon: CreditCard, to: '/admin/payments' },
  { label: 'Analytics', icon: BarChart3, to: '/admin/analytics' },
  { label: 'Activity', icon: ActivityIcon, to: '/admin/activity' },
]

const bottomItems: AdminNavItem[] = [
  { label: 'Settings', icon: Settings, to: '/admin/settings' },
  { label: 'Admin Profile', icon: UserRound },
  { label: 'Log Out', icon: LogOut },
]

function InertNavRow({ label, icon: Icon }: AdminNavItem) {
  return (
    <button
      type="button"
      disabled
...
function NavEntry({ item, pathname }: { item: AdminNavItem; pathname: string }) {
  if (item.to) {
    return (
      ...
      <div className="flex h-full flex-col px-5 pt-5 pb-4 lg:px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-sm font-bold text-slate-900">Alex &middot; Admin Portal</p>
          </div>
        </div>
        ...
        <div className="relative mt-3 lg:hidden">
          <nav ref={mobileNavRef} aria-label="Admin portal" className="no-scrollbar flex gap-1 overflow-x-auto pb-1">
        ...
        {/* Desktop — main items fill the column, Settings/Admin Profile/Log Out pinned at the bottom */}
        <nav aria-label="Admin portal" className="mt-3 hidden flex-1 flex-col gap-0.5 lg:flex">
          {navItems.map((item) => (
            <NavEntry key={item.label} item={item} pathname={pathname} />
          ))}
        </nav>
